...
const AdminBundleForm = () => {
  ...
  const fetchBundle = async () => {
    ...
    try {
      ...
      setError('Error fetching bundle data');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isEdit) {
        await api.put(`/bundles/${id}`, formData);
      ...
      // Navigate back with refresh state immediately
      navigate('/admin/bundles', { replace: true, state: { refresh: true } });
    } catch (error) {
      const errorMessage = error.response?.data?.message || error.message || 'Error saving bundle';
  ...
  if (fetchLoading) {
    return (
      <div className="text-center py-5">
        ...
        <p className="mt-2">Loading bundle data...</p>
      </div>
    );
  }
      ...
      <div className="page-header">
        <div className="page-header-content">
          <div className="page-header-text">
            <h1 className="page-title1">{isEdit ? 'Edit Bundle' : 'Add New Bundle'}</h1>
          </div>
        </div>
      </div>
        ...
        <Col lg={8} xl={6}>
          <div className="categories-table-card">
            <div className="table-header">
              <h5 className="table-title">{isEdit ? 'Update Bundle Information' : 'Create New Bundle'}</h5>
            </div>
            <div className="table-body" style={{ padding: '1.5rem' }}>
              ...
              <Form onSubmit={handleSubmit}>
                ...
                <div className="mb-3">
                  <h5 className="mb-3">Bundle Products ({formData.crackers.length})</h5>
                ...
                <div className="d-flex gap-2">
                  ...
                  <Button variant="secondary" onClick={() => navigate('/admin/bundles')}>
                    Cancel
                  </Button>
                </div>
              </Form>
